<template>
  <div class="flex justify-between py-4 xl:px-40 px-20 items-center">
    ...
    <ul class="flex grow justify-end">
      <li v-for="(item, index) in menu" :key="index" class="px-6">
        <nuxt-link class="block h-full text-second-500 hover:text-primary-500 transition ease-in-out" :to="item.path">
          {{ item.text }}
        </nuxt-link>
      </li>
    </ul>
